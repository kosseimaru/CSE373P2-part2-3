...
public class TfIdfAnalyzer {
    ...
    private IDictionary<String, Double> computeIdfScores(ISet<Webpage> pages) {
        ...
        //IDictionary<String, Double> oldDict = new ChainedHashDictionary<>();
        int size = pages.size();
        for (Webpage page : pages) {
            IList<String> words = page.getWords();
            ISet<String> record = new ChainedHashSet<>();
            
            // both methods work here, I am currently using what TA recommended.
            
            /*oldDict = new ChainedHashDictionary<>();      
            for (KVPair<String, Double> pair : dict) {
                oldDict.put(pair.getKey(), 0.0);
            }
            for (String next : words) {
                if (!oldDict.containsKey(next) && !dict.containsKey(next)) {
                    dict.put(next, 1.0);
                }
                if (oldDict.containsKey(next) && dict.containsKey(next)) {
