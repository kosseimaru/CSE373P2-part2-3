...
public class PageRankAnalyzer {
    ...
    private IDictionary<URI, ISet<URI>> makeGraph(ISet<Webpage> webpages) {
        IDictionary<URI, ISet<URI>> dict = new ArrayDictionary<>();
    ...
    private IDictionary<URI, Double> makePageRanks(IDictionary<URI, ISet<URI>> graph,
        ...
        int size = graph.size();
        IDictionary<URI, Double> rank = new ArrayDictionary<>();
        IDictionary<URI, Double> newRank = new ArrayDictionary<>();
        ...
        for (int i = 0; i < limit; i++) {
            ...
            for (KVPair<URI, ISet<URI>> pair : graph) {
                ...
                ISet<URI> set = pair.getValue();
                int numberOfLinks = set.size();
                if (numberOfLinks == 0) {
                    numberOfLinks = size;
                }
                for (URI otherName : set) {
